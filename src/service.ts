...
interface IExtensionConfig extends vscode.WorkspaceConfiguration, IBaseConfig {}
...
export class Service {
  ...
  async setConfiguration(config: IExtensionConfig) {
    try {
      if (!config.baseUrl) {
        throw new Error(
          'Missing configuration jira-issue.baseUrl. Please update vscode settings, you can open command pallete (F1 or CMD + SHIFT + P) and search for jira baseUrl.'
        );
      }
      if (!config.bearer) {
        if (config.username && !config.password) {
          throw new Error(
            'Missing configuration jira-issue.password. Please update vscode settings, you can open command pallete (F1 or CMD + SHIFT + P) and search for jira password.'
          );
        }

        if (!config.username) {
          throw new Error(
            'Missing configuration jira-issue.bearer(You can also use username). Please update vscode settings, you can open command pallete (F1 or CMD + SHIFT + P) and search for jira bearer.'
          );
        }
      }
  ...
  updateAssignee = async (issueKey: string, assigneeName?: string) => {
    ...
    await this.api?.updateAssignee(issueKey, assigneeName);
  };
    ...
    return this.api
      ...
      .then(async (issue) => {
        ...
        return new Promise((resolve, reject) => {
          vscode.window.showQuickPick(transitionNames, {
            placeHolder: 'Please select the state to be changed',
            onDidSelectItem: (item) => {
              if (item === 'cancel') {
                return;
              }
              //   const id = typeof item === 'string' ? item : item.;

              return this.api
                ?.transitionIssue(issueKey, {
                  transition: { id: item.id },
                })
                .then(() => {
                  // @ts-ignore
                  vscode.window.showInformationMessage(
                    `Successfully updated sub-task ${issueKey} status to ${item.name}`
                  );
